            ...
            if (AP_BUCKET_IS_RESPONSE(e)) {
                ...
                if (resp->status >= 200) {
                    ...
                    if (AP_STATUS_IS_HEADER_ONLY(resp->status)) {
                        /* A header-only final response (e.g. 204/304) is complete
                         * without a body EOS; treat it as complete so c2_process()
                         * does not abort/reset it as "incomplete", which would
                         * re-open PR 69580. */
